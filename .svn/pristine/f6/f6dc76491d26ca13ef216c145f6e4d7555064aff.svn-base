 ...
 */
int bitCount(int x) {
int acc, i; //counter for 1s and mask (calling it iterator here)
  i  = 1;
  i += i << 8; 
  i += i << 16;
  acc  = x & i;
...
int byteSwap(int x, int n, int m) {
  ...
  int tmp_n = (x >> mbits) & 0xFF; //11111111, lowest bit 
...
int greatestBitPos(int x) {
  ...
  if_neg = neg_x_switch & how_neg; 
  if_pos = (~neg_x_switch & ((how_pos >> 1) + 1)); 

  return if_neg | if_pos;
}
...
int isAsciiDigit(int x) {
  ...
  int lowerbound = x + (~0x2F); //x + 010000
  int lowsign = lowerbound >> 31; 
  int upperbound = 0x3A + (~x); //11101 + -(x+1)
  int uppsign = upperbound >> 31;
  
  return !(uppsign | lowsign);
}
